...
var Bundler = class extends bundler.MastraBundler {
  ...
  async writeInstrumentationFile(outputDirectory, customInstrumentationFile) {
    ...
    const __dirname = path.dirname(url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-VHQHPXQQ.cjs', document.baseURI).href))));
  ...
  createBundler(inputOptions, outputOptions) {
    return chunk4KA7G7H5_cjs.createBundler(inputOptions, outputOptions);
  }
  async analyze(entry, mastraFile, outputDirectory, { enableEsmShim = true } = {}) {
    return await chunkCVJ35R7W_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap = false, enableEsmShim = true } = {}) {
    const { workspaceRoot } = await chunkCVJ35R7W_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    const inputOptions = await chunk4KA7G7H5_cjs.getInputOptions(
      mastraEntryFile,
      analyzedBundleInfo,
      "node",
      {
  ...
  getAllToolPaths(mastraDir, toolsPaths = []) {
    const normalizedMastraDir = chunkCVJ35R7W_cjs.slash(mastraDir);
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    let sourcemap = false;
    try {
      const bundlerOptions = await chunkCVJ35R7W_cjs.getBundlerOptions(mastraEntryFile, analyzeDir);
    ...
    try {
      ...
      analyzedBundleInfo = await chunkCVJ35R7W_cjs.analyzeBundle(
    ...
    try {
      const result = await chunkVAW2CQ6L_cjs.writeTelemetryConfig(mastraEntryFile, path.join(outputDirectory, this.outputDir));
    ...
    for (const dep of analyzedBundleInfo.externalDependencies) {
      try {
        ...
        const rootPath = await chunkCVJ35R7W_cjs.getPackageRootPath(dep);
...
//# sourceMappingURL=chunk-VHQHPXQQ.cjs.map
//# sourceMappingURL=chunk-VHQHPXQQ.cjs.map