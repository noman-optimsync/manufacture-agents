...
// src/deploy/base.ts
var Deployer = class extends chunkVHQHPXQQ_cjs.Bundler {
...
function removeAllOptionsExceptDeployer(result, logger) {
  return chunkCVJ35R7W_cjs.removeAllOptionsFromMastraExcept(result, "deployer", logger);
}

// src/build/deployer.ts
async function getDeployer(entryFile, outputDir, logger) {
  const result = await chunkCVJ35R7W_cjs.extractMastraOption(
    "deployer",
    entryFile,
