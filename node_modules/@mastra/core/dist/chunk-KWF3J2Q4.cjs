...
var MastraScorer = class _MastraScorer {
  ...
  toMastraWorkflow() {
    const workflowSteps = this.steps.map((scorerStep) => {
      return chunkXDMQQZNX_cjs.createStep({
        id: scorerStep.name,
    ...
    const workflow = chunkXDMQQZNX_cjs.createWorkflow({
  ...
  async executePromptStep(scorerStep, tracingContext, context) {
    ...
    const resolvedModel = await chunkXWGHD4C7_cjs.resolveModelConfig(modelConfig);
    const judge = new chunkXDMQQZNX_cjs.Agent({
      name: "judge",
      model: resolvedModel,
      instructions,
    ...
    if (scorerStep.name === "generateScore") {
      ...
      if (resolvedModel.specificationVersion === "v2") {
        result = await chunkXDMQQZNX_cjs.tryGenerateWithJsonFallback(judge, prompt, {
          structuredOutput: {
            schema
          },
          tracingContext
...
function isWorkflow(target) {
  return target instanceof chunkXDMQQZNX_cjs.Workflow;
}
...
//# sourceMappingURL=chunk-KWF3J2Q4.cjs.map
//# sourceMappingURL=chunk-KWF3J2Q4.cjs.map