...
async function validateAndSaveScore(storage, payload) {
  const payloadToSave = chunk46XGIEXM_cjs.saveScorePayloadSchema.parse(payload);
...
exports.Mastra = class Mastra {
  ...
  constructor(config) {
    ...
    const workflowEventProcessor = new chunkPIH5FBNQ_cjs.WorkflowEventProcessor({
      mastra: this
    });
    ...
    if (config?.observability) {
      chunkIAJHRFO4_cjs.setupAITracing(config.observability);
    }
...
`
  ...
  registerAITracingExporters() {
    const allTracingInstances = chunkIAJHRFO4_cjs.getAllAITracing();
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-RUDYHG7J.cjs').then(async ({
        GatewayRegistry
      }) => {
   ...
   */
  async shutdown() {
    await chunkIAJHRFO4_cjs.shutdownAITracingRegistry();
...
//# sourceMappingURL=chunk-ICF3MCIN.cjs.map
//# sourceMappingURL=chunk-ICF3MCIN.cjs.map